...
body::before{{content:'';position:fixed;inset:0;z-index:-1;opacity:.35;pointer-events:none;background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 512 512' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='.85' numOctaves='6' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='.5'/%3E%3C/svg%3E");background-size:512px 512px}}
...
/* ─── DIVIDERS ─── */
.section-divider{{display:flex;align-items:center;justify-content:center;gap:16px;padding:40px 24px;opacity:.15;font-family:'Instrument Serif',serif;font-size:13px;color:var(--purple);letter-spacing:6px;text-transform:uppercase}}
.section-divider .line{{flex:1;max-width:120px;height:1px;background:var(--purple)}}
.section-divider .icon{{font-size:16px;animation:spinSlow 12s linear infinite;display:inline-block}}
@keyframes spinSlow{{from{{transform:rotate(0deg)}}to{{transform:rotate(360deg)}}}}

/* ─── FULL-BLEED PURPLE BAND ─── */
.purple-band{{background:linear-gradient(135deg,#7a3d73,#974e8e,#b86aae);padding:80px 24px;margin:0;width:100%;position:relative;overflow:hidden}}
.purple-band .band-inner{{max-width:800px;margin:0 auto;text-align:center;position:relative;z-index:1}}
.purple-band .band-symbol{{font-family:'Instrument Serif',serif;font-size:clamp(60px,10vw,120px);color:rgba(255,255,255,.06);line-height:1;margin-bottom:-30px}}
.purple-band h2{{font-size:clamp(24px,3vw,38px);font-weight:700;color:#fff;letter-spacing:-1px;margin-bottom:12px;line-height:1.15}}
.purple-band p{{color:rgba(255,255,255,.7);font-size:14px;line-height:1.7;max-width:580px;margin:0 auto 28px}}
.purple-band .band-btn{{padding:14px 36px;border-radius:var(--pill);background:rgba(255,255,255,.15);color:#fff;border:1px solid rgba(255,255,255,.2);font-size:13px;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:10px;transition:background .3s,transform .3s}}
.purple-band .band-btn:hover{{background:rgba(255,255,255,.25);transform:translateY(-2px)}}
.purple-band::before{{content:'';position:absolute;inset:0;background:repeating-linear-gradient(90deg,transparent,transparent 40px,rgba(255,255,255,.02) 40px,rgba(255,255,255,.02) 41px);pointer-events:none}}
.purple-band .orb{{position:absolute;width:240px;height:240px;border-radius:50%;border:1px solid rgba(255,255,255,.04);left:50%;top:50%;transform:translate(-50%,-50%);pointer-events:none}}
...
/* Fail-open visibility: if JS or CDN imports fail, content must still render. */
.hero-badge,.hero h1,.hero-sub,.hero-desc,.hero-actions,.hero-platform,.section-label,.section-title,.section-desc,.stats-grid,.loop-cycle,.stack-card,.strategy-card,.table-wrap,.about-card{{opacity:1}}
.reveal{{opacity:1!important;transform:none!important}}
.animate .hero-badge,.animate .hero h1,.animate .hero-sub,.animate .hero-desc,.animate .hero-actions,.animate .hero-platform,.animate .section-label,.animate .section-title,.animate .section-desc,.animate .stats-grid,.animate .loop-cycle,.animate .stack-card,.animate .strategy-card,.animate .table-wrap,.animate .about-card{{opacity:0}}
.animate .reveal{{opacity:0!important;transform:translateY(28px)!important}}
.animate .reveal.visible{{opacity:1!important;transform:translateY(0)!important}}
...
<!-- DIVIDER -->
<div class="section-divider"><span class="line"></span><span class="icon">⟳</span><span class="line"></span></div>

<!-- STATS -->
<div style="padding:0 24px;max-width:1100px;margin:0 auto;position:relative">
<p class="section-label reveal" style="text-align:center">By the Numbers</p>
<h2 class="section-title reveal reveal-1" style="text-align:center;max-width:none">Measurable <span class="gt">Loop Impact</span></h2>
<p class="section-desc reveal reveal-2" style="text-align:center;margin:0 auto 48px">Real results from the first 8 weeks of full deployment.</p>
</div>
...
<div class="band-symbol">∞</div>
<h2>The Loop <em style="font-style:normal;font-weight:300;color:rgba(255,255,255,.5)">in</em> Action</h2>
<p>Four engines running in continuous iteration — each cycle sharpens the output, expands intelligence, and accelerates growth.</p>
<a href="#strategy" class="band-btn">Explore the System ↻</a>
</div>
<div class="orb"></div>
<div class="orb" style="width:400px;height:400px;border-color:rgba(255,255,255,.02)"></div>
</div>
...
for (let r = 0; r < 3; r++) {{
const tringGeo = new THREE.TorusGeometry(2.2 + r * 0.6, 0.015, 16, 60);
...
tring.position.y = 0.3;
tring.rotation.x = Math.PI * 0.5 + (r - 1) * 0.15;
tring.userData = {{ speed: 0.1 + r * 0.05, tilt: (r - 1) * 0.15 }};
scene.add(tring);
}}
...
/* Rotate knot */
/* Ground ring pulse */
groundRing.scale.setScalar(1 + Math.sin(t * 0.3) * 0.02);
